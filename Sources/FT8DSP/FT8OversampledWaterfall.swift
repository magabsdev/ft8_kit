...
public struct FT8OversampledWaterfallConfiguration: Equatable, Sendable {
    ...
    public init(
        sampleRate: Float,
        symbolSamples: Int = 1_920,
        timeOversampling: Int = 4,
        frequencyOversampling: Int = 2,
    ...
    public var hopSamples: Int {
        symbolSamples / timeOversampling
    }

    public var fftSize: Int {
        symbolSamples * frequencyOversampling
    }

    public var baseBinWidth: Float {
        sampleRate / Float(symbolSamples)
    }

    public var oversampledBinWidth: Float {
    ...
    func validate() throws {
        guard sampleRate > 0,
              symbolSamples >= 2,
              timeOversampling > 0,
              frequencyOversampling > 0,
              symbolSamples.isMultiple(of: timeOversampling),
              hopSamples > 0,
              minimumFrequency >= 0,
              maximumFrequency > minimumFrequency,
              maximumFrequency <= sampleRate / 2 else {
...
/// Protocol-specific FT8 waterfall storage matching ft8_lib's logical layout:
/// [block][time subdivision][frequency subdivision][base-frequency bin].
public struct FT8OversampledWaterfall: Equatable, Sendable {
    ...
    public init(
        sampleRate: Float,
        symbolSamples: Int,
        timeOversampling: Int,
        frequencyOversampling: Int,
        blockCount: Int,
        frameCount: Int,
        baseBinCount: Int,
        firstBaseBin: Int,
    ...
    public var blockStride: Int {
        timeOversampling * frequencyOversampling * baseBinCount
    }

    public var timeSubdivisionStride: Int {
        frequencyOversampling * baseBinCount
    }

    public var frequencySubdivisionStride: Int {
        baseBinCount
    }
    ...
    public func magnitude(
        block: Int,
        timeSubdivision: Int,
        frequencySubdivision: Int,
        baseBin: Int
    ) -> Float? {
        guard block >= 0, block < blockCount,
              timeSubdivision >= 0, timeSubdivision < timeOversampling,
              frequencySubdivision >= 0,
              frequencySubdivision < frequencyOversampling,
              baseBin >= 0, baseBin < baseBinCount else {
            return nil
        }
        ...
        let index = block * blockStride
            + timeSubdivision * timeSubdivisionStride
            + frequencySubdivision * frequencySubdivisionStride
            + baseBin
    ...
    public func frequency(
        ...
        minimumFrequency
            + Float(baseBin) * baseBinWidth
            + Float(frequencySubdivision) * oversampledBinWidth
    }
}
