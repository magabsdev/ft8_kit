...
public extension FT8OptimizedDecoder {
    func decode(
        ...
        let refined = found.map {
            candidateRefiner.refine($0, in: oversampledWaterfall)
        }

        return try decodePreparedCandidates(
            refined,
            spectrogram: spectrogram,
    ...
    private func decodePreparedCandidates(
        ...
        for (index, candidate) in scheduled.enumerated() {
            ...
            guard soft.averageConfidence >=
                    configuration.minimumSoftSymbolConfidence else {
                continue
            }
    ...
    private func deduplicatePrepared(
        _ decodes: [FT8CompleteDecode]
    ) -> [FT8CompleteDecode] {
        var accepted: [FT8CompleteDecode] = []

        for decode in decodes.sorted(
            by: { $0.decoded.confidence > $1.decoded.confidence }
        ) {
